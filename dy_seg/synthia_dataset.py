...
class SegDataset():
    ...
    def half_crop_w_context(self, half, context, pc, rgb, semantic, center):
        ...
        if half == 0:
            sample_idx_half_w_context = all_idx
            pc_half_w_context = pc[sample_idx_half_w_context]
            loss_mask = pc_half_w_context[:, 2] > -200000.0

            #print("####data")
            #un1, indices1 = np.unique(loss_mask, return_index=True)
            #print(un1)
        else:
            sample_idx_half_w_context = all_idx[0]
            pc_half_w_context = pc[sample_idx_half_w_context]
