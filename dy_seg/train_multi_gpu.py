...
parser.add_argument('--data', default='/data/hejingming/PCL/meteornet-master/processed_pc', help='Dataset dir [default: model_basic]')
parser.add_argument('--model', default='dy_seg_model', help='Model name [default: model_basic]')
...
parser.add_argument('--max_epoch', type=int, default=150, help='Epoch to run [default: 100]')
parser.add_argument('--batch_size', type=int, default=12, help='Batch Size during training [default: 32]')
parser.add_argument('--learning_rate', type=float, default=0.0016, help='Initial learning rate [default: 0.001]')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/cpu:0'):
            ...
            MODEL.get_model(pointclouds_pl, NUM_FRAME, is_training_pl, bn_decay=bn_decay)
            ...
            for i in range(NUM_GPUS):
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    with tf.device('/gpu:%d'%(i)), tf.name_scope('gpu_%d'%(i)) as scope:
                        ...
                        labelweights_batch = tf.slice(labelweights_pl,
                            [i*DEVICE_BATCH_SIZE,0], [DEVICE_BATCH_SIZE,-1])
                        masks_batch = tf.slice(masks_pl,
                            [i*DEVICE_BATCH_SIZE,0], [DEVICE_BATCH_SIZE,-1])


                        pred, end_points = MODEL.get_model(pc_batch, NUM_FRAME, is_training_pl, bn_decay=bn_decay)

                        MODEL.get_loss(pred, label_batch, masks_batch, end_points, labelweights_batch)
                        losses = tf.get_collection('losses1', scope)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        for half in [0]:
            ...
            #ply1=batch_data[0]
            #for i in range(ply1.shape[0]):
            #    print('v '+str(ply1[i][0])+' '+str(ply1[i][1])+' '+str(ply1[i][2])+' 6')
